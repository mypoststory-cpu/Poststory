    ...
    const fetchImages = async () => {
      ...
      try {
        const response = await axios.get(
          `https://res.cloudinary.com/${cloudName}/image/list/funny_folder.json`
        );
...
const handleImageClick = (imgSrc) => {
 ...
 const filteredPosts = images.filter((img) => {
  ...
  if (query !== "") return pId.includes(query);
  ...
  if (tabLower.includes("sarcasm")) {
   
    return pId.includes("sarcasm") || pId.includes("wit");
  }

  const cleanTab = tabLower.replace(/ /g, "_");
    ...
    <div className="cat-page-container6">
      ...
      <div className="cat-filter-header6">
        ...
        <div className="right-side6">
          <div className="filter-controls6" onClick={() => setShowFilter(!showFilter)}>
      ...
      <main className="posts-grid6">
        {loading ? (
          ...
          filteredPosts.map((post) => {
            ...
            return (
              <div key={post.public_id} className="post-item6" style={{ position: 'relative' }}>
                <img 
                  src={imageUrl} 
                  alt="Funny Post"
                  className="grid-img6"
          ...
          <p className="no-images-msg" style={{ textAlign: "center", gridColumn: "1/-1" }}>No images found...</p>
        )}
      </main>
