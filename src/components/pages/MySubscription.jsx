...
export default function MySubscription() {
    ...
    const getSubscriptionData = async () => {
      ...
      try {
        ...
        if (historyData.length > 0) {
          ...
          // CONDITION ADDED: Check if latest subscription actually has a valid payment ID
          if (latestPaidSub.paymentId && latestPaidSub.paymentId.trim() !== "") {
    ...
    try {
      ...
      const options = {
        ...
        handler: async function (response) {
          // CONDITION ADDED: Validate payment ID received from Razorpay handler
          if (!response || !response.razorpay_payment_id) {
            alert("Payment failed: Invalid Payment ID received.");
            setIsProcessing(false);
            return;
          }
  ...
  const finalizeSubscription = async (paymentId) => {
    // CONDITION ADDED: Safety check before writing to DB
    if (!paymentId) {
      alert("Cannot finalize subscription without a valid Payment ID.");
      setIsProcessing(false);
      return;
    }
    ...
    try {
      ...
      let startYY = String(fyStartYear).slice(-2);
      let endYY = String(fyEndYear).slice(-2);
      const financialYearStr = `${startYY}/${endYY}`;
      ...
      if (!querySnapshot.empty) {
        const existingInvoices = querySnapshot.docs.map(docSnap => docSnap.data().invoiceNumber || "");
        const currentFyInvoices = existingInvoices.filter(inv => inv && inv.includes(financialYearStr));
        
        if (currentFyInvoices.length > 0) {
          const latestInv = currentFyInvoices[0];
          const parts = latestInv.split("-");
      ...
      const formattedSeq = String(nextSequenceNum).padStart(5, '0');
      const customInvoiceNumber = `#PS-${financialYearStr}-${formattedSeq}`; 
      ...
      const subDocData = {
        ...
        gstAmount: totalTax,
        paymentId: paymentId, // Storing verified Payment ID
        state: userState,
      ...
      docPdf.text(customInvoiceNumber, 195, 28, { align: "right" });
      ...
      docPdf.text(`Rs. ${totalPrice}.00`, 149, 47);
      ...
      docPdf.text("Paid", 195, 85, { align: "right" });
      ...
      docPdf.save(`Invoice_${customInvoiceNumber.replace(/[\/#]/g, "")}_${userData.mobile}.pdf`);
  ...
  const generateAndOpenInvoicePDF = (item) => {
    // CONDITION ADDED: Prevent generating or viewing invoice if paymentId is missing
    if (!item.paymentId || item.paymentId.trim() === "") {
      alert("Invalid Action: Invoice cannot be generated for records without a valid Payment ID.");
      return;
    }
    ...
    docPdf.setFontSize(10);
    docPdf.text(item.invoiceNumber || `#PS2627-00001`, 195, 28, { align: "right" });
    ...
    docPdf.setFont("helvetica", "bold");
    docPdf.setTextColor(15, 23, 42);
    docPdf.text(`Rs. ${item.amount || totalPrice}.00`, 149, 47);
    ...
    docPdf.setFontSize(10);
    docPdf.text(`Customer Name: ${userData.name || "Valued Customer"}`, 15, 85);
    docPdf.text(`Place of Supply: ${item.state || userState}`, 15, 90);
    ...
    docPdf.setTextColor(0, 0, 0);
    docPdf.text(item.date || new Date().toLocaleDateString('en-IN'), 195, 79, { align: "right" });
    docPdf.text("Paid", 195, 85, { align: "right" });
    docPdf.setFontSize(8.5);
    docPdf.text(item.paymentId, 195, 90, { align: "right" });
    ...
    docPdf.setFontSize(9.5);
    docPdf.text("1.00 pcs", 125, 114, { align: "right" });
    docPdf.text(`${item.baseAmount || baseAmount}`, 155, 114, { align: "right" });
    docPdf.text(`${item.baseAmount || baseAmount}`, 192, 114, { align: "right" });
    ...
    docPdf.setTextColor(0, 0, 0);
    docPdf.text(`${item.baseAmount || baseAmount}`, 195, 135, { align: "right" });
    if (isSameState) {
      docPdf.text(`${(totalTax / 2).toFixed(2)}`, 195, 141, { align: "right" });
      docPdf.text(`${(totalTax / 2).toFixed(2)}`, 195, 147, { align: "right" });
    } else {
                  ...
                  subDetails.history.map((item) => (
                    <div key={item.id} className="history-card" style={{ background: '#111', padding: '15px', borderRadius: '8px', marginBottom: '12px', border: '1px solid #222' }}>
                      <div className="history-main" style={{ display: 'flex', justifyContent: 'space-between', fontWeight: 'bold', color: '#fff' }}>
                        <span className="hist-plan">{item.plan} {item.invoiceNumber && <small style={{ color: '#dbbc2f', fontWeight: 'normal' }}>({item.invoiceNumber})</small>}</span>
                      ...
                      <div className="history-footer" style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px', color: '#888', marginTop: '5px' }}>
                        ...
                        {/* CONDITION ADDED: Show success only if Payment ID exists */}
                        <span style={{ color: item.paymentId ? '#4caf50' : '#ff9800', fontWeight: 'bold' }}>
                          {item.paymentId ? "Success" : "Pending/No ID"}
                        </span>
                      </div>

                      {/* View Invoice PDF Button - Only shows/works if paymentId exists */}
                      {item.paymentId && (
                        <div style={{ marginTop: '12px', paddingTop: '10px', borderTop: '1px solid #222', display: 'flex', justifyContent: 'flex-end' }}>
                          <button 
                            onClick={() => generateAndOpenInvoicePDF(item)}
                            style={{
                              padding: '6px 99px',
                              background: '#333',
                              color: '#dbbc2f',
