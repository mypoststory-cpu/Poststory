...
const [selectedLanguage, setSelectedLanguage] = useState("all_lang");
  ...
  useEffect(() => {
    const fetchImages = async () => {
      ...
      try {
        ...
        console.error("Trending images load error:", error);
      } finally {
        setLoading(false);
      }
    };
 ...
 const filteredPosts = images.filter((img) => {
  ...
  if (query !== "") {
    ...
    return keywords.some(word => pId.includes(word));
  }
  ...
  } else if (activeTab === "Good Vibes") {
    matchesCategory = pId.includes("vibes");
  } else if (activeTab === "Golden Hour") {
    matchesCategory = pId.includes("hour") ;
  } else if (activeTab === "Daily") {
    matchesCategory = pId.includes("daily");
  }
    ...
    <Navbar onSearch={(text) => setSearchQuery(text)} />
      ...
      <div className="cat-filter-header8">
        <div className="left-side8">
          <img src={backArrow} alt="back" className="back-icon8" onClick={() => window.history.back()} />
  ...
  >
   <option value="All">All</option>
      ...
      <div className="tab-container8">
        {["All", "Good Vibes", "Golden Hour", "Daily"].map((tab) => (
          <button 
            key={tab}
